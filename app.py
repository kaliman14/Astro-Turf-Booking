    ...
    if request.method == "POST":
        ...
        def isAvailable(test_date, test_time):
            ...
            for i in range(0, len(timeslot)):
                ...
                if (
                    (time_obj <= test_time_obj and test_time_obj <= time2_obj)
                    and test_date_obj == ndate_obj
                ) or test_date_obj < date_now_obj:
                    return False
            return True
...
def admin():
    ...
    cursor.execute(
        "SELECT id, name, phone, date, starttime, endtime, cost FROM bookings"
    )
...
def login():
    if request.method == "POST":
        ...
        try:
            ...
            flash(
                f"Error logging in. Please try again later. \nError code: {e}", "error"
            )
            return redirect(url_for("login"))
...
def available_slots():
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT * FROM timeslots WHERE date = ? AND is_available = 1", (date,)
    )
    available_slots = cursor.fetchall()
    conn.close()
...
def booking_history():
    ...
    try:
        ...
        """,
            (session.get("user_id"),),
        )  # Adjust based on how you store user ID in the session
